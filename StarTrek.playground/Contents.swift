...
let humanFemaleCount = humanOfficers.filter{ femaleOfficers.contains($0)}.count



// Q2: How many first officers are human?
let humanFirstCount = firstOfficers.filter{ humanOfficers.contains($0.key)}.count
...
let SortedCaptains: [Officer] = allCaptains.sorted()
print(SortedCaptains.map{$0.name})
...
var bothCaptains: [String: String] = firstOfficers.filter{ vulcanOfficers.contains($0.key) }
let bothOfficers: [Officer] = bothCaptains.map{Officer(name: $0, ship: $1)}
print(bothOfficers.map{$0.name})
...
let vulcanCount = bothOfficers.filter{$0.name.count < 5}.count
print("There are \(vulcanCount) officers have names shorter than 5 characters")
